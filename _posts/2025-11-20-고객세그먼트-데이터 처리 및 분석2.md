...
동일 코호트 내에서도 RFM 세그먼트별 유지 패턴의 차이를 확인할 수 있도록 설계했습니다.

---

#### cohort_stat (대시보드 사용)
전체 고객을 기준으로 코호트별 유지 지표를 산출했습니다.
...
고객을 RFM 세그먼트 단위로 분리하여 코호트 내 유지율의 차이를 확인했습니다.
...
Recency 중심의 단순하고 직관적인 이탈 분류 기준을 사용했습니다.
이를 통해 **이탈 고객 규모, 행동 패턴, 매출 손실 리스크**를 명확히 파악할 수 있습니다.

---

#### monthly_churn_stats
Recency를 기반으로 고객을 다음 4개 그룹으로 분류했습니다.
...
#### churn_user_ltv_stat
2020년 4월을 기준으로 이탈 고객의 과거 가치와 앞으로의 손실을 계산했습니다.
...
세그먼트별 **조회/장바구니/구매 전환 지표**를 산출했습니다.
...
각 세그먼트가 최근 3일 / 7일 / 14일 / 전체(all day) 동안 발생시킨 행동 수를 분석했습니다.
...
월별 세그먼트 이동(Transition)을 기반으로 이동인원수를 산출했습니다.
...
이탈 고객이 어떤 카테고리를 많이 구매했는지 산출했습니다.
